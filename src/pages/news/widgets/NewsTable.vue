...
const emit = defineEmits<{
  (event: 'edit-new', item: New): void
  (event: 'delete-new', item: New): void
...
const onUserDelete = async (item: New) => {
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          icon="mso-edit"
          aria-label="Edit new"
          @click="$emit('edit-new', rowData as New)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="onUserDelete(rowData as New)"
        />
      </div>
    </template>
  </VaDataTable>
